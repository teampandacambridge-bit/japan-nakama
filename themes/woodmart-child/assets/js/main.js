...
window.addEventListener('load', function () {
    ...
    function homepageTopBannerAd() {
        const sections = document.querySelectorAll('.ad-animated-hompage-top');
        if (!sections.length) return;

        sections.forEach(section => {
            const imgBase = section.dataset.themeUri + '/assets/ads/homepage-top-banner/';

            const images = {
                sm: [1, 2, 3, 4, 5].map(n => `${imgBase}300x600_${n}.webp`),
                md: [1, 2, 3, 4, 5].map(n => `${imgBase}750x250_${n}.webp`),
                lg: [1, 2, 3, 4, 5].map(n => `${imgBase}970x250_${n}.webp`),
            };

            const imgSm = section.querySelector('.ad-img--sm');
            const imgMd = section.querySelector('.ad-img--md');
            const imgLg = section.querySelector('.ad-img--lg');

            let currentIndex = 0;

            function showImage() {
                if (imgSm) imgSm.src = images.sm[currentIndex];
                if (imgMd) imgMd.src = images.md[currentIndex];
                if (imgLg) imgLg.src = images.lg[currentIndex];
    ...
    function sidebarVerticalAd() {
        const sections = document.querySelectorAll('.ad-sidebar-vertical');
        if (!sections.length) return;

        sections.forEach(section => {
            const imgBase = section.dataset.themeUri + '/assets/ads/homepage-top-banner/';
            const images = [1, 2, 3, 4, 5].map(n => `${imgBase}300x600_${n}.webp`);
