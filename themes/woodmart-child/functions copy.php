...
// require_once get_stylesheet_directory() . '/inc/custon-functions/enqueue-scripts.php';

// add_action('wp_enqueue_scripts', 'enqueue_scripts', 10010);
...
{
    // Check if we're inside a single post and the content has h2 tags.
    if (is_singular() && false !== strpos($content, '<h2')) {
        // The shortcode to be inserted, with the specified attributes.
        $ad_shortcode = '[adsanity_group align=aligncenter num_ads=1 num_columns=1 group_ids=13189]';
        // Process the shortcode.
        $ad_code = do_shortcode($ad_shortcode);

        // Replace the paragraph HTML with Visual Composer shortcodes for spacing.
        $ad_code_with_vc_spacing = $ad_code . '[ultimate_spacer height="40"]';

        // Use preg_replace_callback to insert ad code before every third h2 tag.
        // This pattern looks for the start of h2 tags.
        $pattern = '/(<h2.*?>)/i';

        // Counter to track occurrences of h2
        $counter = 0;

        $content = preg_replace_callback($pattern, function ($matches) use (&$counter, $ad_code_with_vc_spacing) {
            // Increment counter for each h2 found
            $counter++;
            // Insert ad code and VC spacing on every third h2
            if ($counter % 3 == 0) {
                return $ad_code_with_vc_spacing . $matches[0];
            } else {
                // Return the h2 tag unchanged if it's not the third occurrence
                return $matches[0];
            }
        }, $content);
    }
...
{
    if (is_single()) {
        $shortcode = do_shortcode("[adsanity_group align=aligncenter num_ads=1 num_columns=1 group_ids=13189]");
...
function get_cached_latest_posts($limit = 5)
{
