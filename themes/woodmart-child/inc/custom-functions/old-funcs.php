...
function wpb_redirect_attachment_to_media_file()
{
    if (is_attachment()) {
        global $post;
        if ($post && $post->post_parent) {
            // Redirect to parent post if available
            wp_redirect(get_permalink($post->post_parent), 301);
            exit;
        } else {
            // Otherwise, redirect to the image file itself
...
add_action('template_redirect', 'wpb_redirect_attachment_to_media_file');
...
// No Index Product Filters
function custom_noindex_filtered_pages()
{
    // Check if it's the shop, any product archive, or a taxonomy archive for products
    if (is_shop() || is_post_type_archive('product') || is_tax()) {

        // List of query parameter patterns to match
        $filter_params = ['filter_', 'orderby', 'min_price', 'max_price', 'pa_'];
...
add_action('wp_head', 'custom_noindex_filtered_pages');
