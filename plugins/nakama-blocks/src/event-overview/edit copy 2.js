...
export default function Edit({ attributes, setAttributes }) {
	...
	const addItem = () => {
		setAttributes({ items: [...itemsArray, { heading: 'New Heading', description: 'Description' }] });
	};
	...
	const addListItem = () => {
		setAttributes({ list: [...listArray, 'List item'] });
	};
			...
			<ul className="event-overview-extra-list">
				{listArray.map((item, index) => (
					<li key={index}>
						...
						/>
						<Button variant="secondary" isDestructive onClick={() => removeListItem(index)}>
							{__('Remove', 'event-overview')}
						</Button>
					</li>
				))}
